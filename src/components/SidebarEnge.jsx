 ...
 * - Muestra SOLO capas autorizadas para el usuario (si no es admin/super).
 * - Swatch de color con LegendURL (cuando exista).
 * ------------------------------------------------------------------ */
'use client';
...
import { X, LogOut, Layers, MapPinned, ChevronRight, Settings, ShieldAlert } from 'lucide-react';
...
const LayerItem = ({ name, checked, onToggle, legend }) => (
  <label className="flex items-center justify-between px-4 py-1.5 cursor-pointer rounded hover:bg-gray-100">
    <span className="flex items-center gap-2 text-sm">
      <Swatch url={legend}/>
      <span className={cn(checked ? 'text-blue-600 font-medium' : 'text-gray-800', 'truncate')}>
        {name.replace(/^SICDI:/, '')}
      </span>
    </span>
...
const Fold = ({ title, icon:Icon, open, onToggle, children }) => (
  <div className="border-b border-gray-200">
    ...
    <AnimatePresence initial={false}>
      {open && (
        <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} transition={FX} className="overflow-hidden">
          {children}
        </motion.div>
      )}
    </AnimatePresence>
...
export default function SidebarEnge({
  ...
  const [fold, setFold] = useState(() => {
    if (typeof window === 'undefined') return { capas:true, estilos:true, admin:true };
    try { return JSON.parse(localStorage.getItem(LS)) ?? { capas:true, estilos:true, admin:true }; }
    catch { return { capas:true, estilos:true, admin:true }; }
  });
  const toggleFold = k => {
  ...
  return (
    <AnimatePresence>
      {open && (
        <>
          <motion.div className="fixed inset-0 z-40 bg-black/40" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={FX} onClick={onClose}/>
          <motion.aside
            className="fixed left-0 top-0 z-50 flex h-full w-80 max-w-[90%] flex-col bg-white text-gray-800 shadow-2xl"
            initial={{x:'-100%'}} animate={{x:0}} exit={{x:'-100%'}} transition={FX}
          >
            <div className="flex items-center justify-between px-6 py-4 bg-blue-600 text-white">
              <p className="text-base font-semibold">Opciones</p>
              <button onClick={onClose} className="p-1.5 rounded hover:bg-white/20"><X size={18}/></button>
            </div>
            ...
            <div className="flex items-center gap-4 px-5 py-5 border-b border-gray-200">
              ...
              <button onClick={logout} className="flex h-8 w-8 items-center justify-center rounded-full bg-rose-500 text-white hover:bg-rose-600">
                <LogOut size={16}/>
              </button>
            </div>
            ...
            <div>
              <div className="border-b border-gray-200">
                <button onClick={()=>toggleFold('capas')}
                        className="flex w-full items-center justify-between px-5 py-2.5 text-xs font-semibold uppercase tracking-wider text-gray-600 hover:bg-gray-50">
                  <span className="flex items-center gap-2"><Layers size={16}/> CAPAS</span>
                  <motion.span animate={{ rotate: fold.capas ? 90 : 0 }} transition={{ duration:0.2 }}>
                    <ChevronRight size={16}/>
                  </motion.span>
                </button>
                <AnimatePresence initial={false}>
                      ...
                      <div className="sidebar-scrollbar max-h-60 overflow-y-auto">
                        {capasFiltradas.length ? capasFiltradas.map(c=>(
                          ...
                          <div className="px-4 py-2 text-sm text-gray-500">
                            {allowAllLayers === true ? 'No hay capas disponibles.' : 'No tienes capas autorizadas.'}
                          </div>
                        )}
                      </div>
                    </motion.div>
              ...
              <div className="border-b border-gray-200">
                <button onClick={()=>toggleFold('estilos')}
                        className="flex w-full items-center justify-between px-5 py-2.5 text-xs font-semibold uppercase tracking-wider text-gray-600 hover:bg-gray-50">
                  <span className="flex items-center gap-2"><MapPinned size={16}/> ESTILO BASE</span>
                  <motion.span animate={{ rotate: fold.estilos ? 90 : 0 }} transition={{ duration:0.2 }}>
                    <ChevronRight size={16}/>
                  </motion.span>
                </button>
                <AnimatePresence initial={false}>
                  {fold.estilos && (
                    <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} transition={FX} className="overflow-hidden">
                      <StyleChips mapStyles={mapStyles} mapStyleActual={mapStyleActual} cambiarEstiloMapa={cambiarEstiloMapa}/>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
              ...
              {(isAdmin || isSuper) && (
                <>
                  <div className="h-6 border-t border-gray-200"/>
                  <Fold title="Admin" icon={Settings} open={fold.admin} onToggle={()=>toggleFold('admin')}>
                    <div className="px-5 pb-4">
                      <button onClick={()=>{ router.push('/admin/usuarios'); onClose(); }}
